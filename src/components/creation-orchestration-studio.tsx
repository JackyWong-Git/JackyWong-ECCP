        ...
        <section className="space-y-3">
          ...
          <div className="grid min-h-[680px] gap-3 xl:grid-cols-[300px_minmax(420px,1fr)_340px]">
            ...
            <div className="flex min-h-[560px] flex-col overflow-hidden rounded-3xl border border-[#E1E8ED] bg-white shadow-[0_10px_30px_rgba(47,65,82,0.045)]">
              ...
              {result?.runs.some(run => run.approval) ? <button type="button" onClick={() => onNavigate('tasks')} className="m-3 flex h-9 items-center justify-center gap-1.5 rounded-xl bg-[#FFF0ED] text-[8px] font-semibold text-[#C65345]"><ShieldCheck className="h-3.5 w-3.5" />前往 Agent 任务中心审批<ChevronRight className="h-3 w-3" /></button> : null}
            </div>
          </div>
        </section>
      ) : null}
