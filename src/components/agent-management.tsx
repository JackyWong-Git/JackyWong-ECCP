...
export function AgentManagement({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? 'space-y-4' : 'mx-auto max-w-[1440px] space-y-5 p-4 sm:p-6 lg:p-8'}>
      {embedded ? (
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-[16px] font-semibold text-[#253640]">Agent 与能力配置</h2>
            <p className="mt-1 text-[10px] text-[#7B8A95]">配置角色、模型、Prompt、Skill 和 RAG，保存后立即可用于协作方案。</p>
          </div>
          <div className="flex gap-2"><button type="button" onClick={() => void loadData()} className="flex h-9 items-center gap-1.5 rounded-xl border border-[#DDE5EA] bg-white px-3 text-[10px] font-semibold text-[#60707D]"><RefreshCw className="h-3.5 w-3.5" />刷新</button><button type="button" onClick={() => setShowCreate(true)} className="flex h-9 items-center gap-1.5 rounded-xl bg-[#5267E8] px-4 text-[10px] font-semibold text-white"><Plus className="h-3.5 w-3.5" />新建 Agent</button></div>
        </div>
      ) : (
      ...
      <div className={`grid overflow-hidden rounded-3xl border border-[#E1E8ED] bg-white shadow-[0_12px_40px_rgba(37,54,70,0.06)] lg:grid-cols-[330px_minmax(0,1fr)] ${embedded ? 'min-h-[620px]' : 'min-h-[660px]'}`}>
