...
const starters = [
  ...
  { icon: WandSparkles, title: '从需求到脚本', prompt: '我要做一条员工故事视频，请规划从选题、大纲到脚本的完整步骤。' },
];
...
export function MainAgentWorkspace({ onNavigate }: MainAgentWorkspaceProps) {
  ...
  useEffect(() => {
    const saved = window.sessionStorage.getItem('eccp-main-agent-chat');
    if (!saved) return;
    try {
      setMessages(JSON.parse(saved) as ChatMessage[]);
    } catch {
      window.sessionStorage.removeItem('eccp-main-agent-chat');
    }
  }, []);

  useEffect(() => {
    window.sessionStorage.setItem('eccp-main-agent-chat', JSON.stringify(messages.slice(-30)));
    endRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [messages, draft, createdTask]);
  ...
  const send = async (preset?: string) => {
    ...
    setLoading(true);
    try {
      const projectIntent = isProjectIntent(content);
      const result = await executeAgentTask({
        source: 'assistant',
        conversation_id: 'main-agent-workspace',
        web_enabled: true,
        knowledge_enabled: true,
        input_text: projectIntent
          ? [
        ...
        <section className="flex min-h-[680px] min-w-0 flex-col overflow-hidden rounded-[26px] border border-white/90 bg-white shadow-[0_22px_65px_rgba(38,57,72,0.09)]">
          ...
          <footer className="shrink-0 border-t border-[#E8EDF1] bg-white p-3 sm:p-4">
            <div className="mx-auto max-w-3xl rounded-[20px] border border-[#DCE4EA] bg-[#FAFCFD] p-2.5 shadow-[0_8px_24px_rgba(38,57,72,0.05)] focus-within:border-[#AEBBF4]">
              ...
              <div className="flex items-center justify-between px-1"><span className="text-[8px] text-[#96A3AD]">可调用搜索、知识库与多 Agent · 创建和发布前会请你确认</span><button type="button" onClick={() => void send()} disabled={!prompt.trim() || loading} className="ai-gradient flex h-9 w-9 items-center justify-center rounded-xl text-white disabled:opacity-40"><ArrowUp className="h-4 w-4" /></button></div>
            </div>
          </footer>
        </section>
